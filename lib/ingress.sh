...
ingress::fetch-host() {
  if [[ -z "${ingress_host_cache:-}" ]]; then
    ingress_host_cache="$(tools::kubectl -n istio-ingress get service istio-ingressgateway -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null 2>/dev/null).nip.io"
...
ingress::fetch-port() {
  if [[ -z "${ingress_port_cache:-}" ]]; then
    ingress_port_cache="$(tools::kubectl -n istio-ingress get service istio-ingressgateway -o jsonpath='{.spec.ports[?(@.name=="http2")].port}' 2>/dev/null)"
...
ingress::fetch-secure-port() {
  if [[ -z "${secure_ingress_port_cache:-}" ]]; then
    secure_ingress_port_cache="$(tools::kubectl -n istio-ingress get service istio-ingressgateway -o jsonpath='{.spec.ports[?(@.name=="https")].port}' 2>/dev/null)"
